...
# Pfam
df_pfam = pd.DataFrame(columns=["contigId", "genPred_len", "pfam_name", "pfam_id", "pfam_qlen", "pfam_E_value"], dtype=float)
...
# Kegg
df_kegg = pd.DataFrame(columns=["contigId", "genPred_len", "kegg_name", "kegg_id", "kegg_qlen", "kegg_E_value"], dtype=float)
...
# Diammond
df_diammond = pd.DataFrame(columns=["contigId", "diam_name", "diam_seq_identity", "diam_aln_len", "diam_E_value"], dtype=float)
...
df_annot.insert(0, 'contigId', df_annot.pop('contigId'))
