...
class ClusterSeq:
    ...
    def cluster_annotation(self, annotation_table: str | Path, taxo_annotation = None) -> None:
        """
        ...
        if taxo_annotation != None:
            taxon_df = pl.read_csv(taxo_annotation, separator='\t')
            contig_names = df_join['seq_id'].str.replace(pattern=r"(_[0-9]*$)", value='')
            contig_names = contig_names.rename("contigId")
            df_join = df_join.insert_column(1, contig_names)
            df_join = df_join.join(taxon_df, left_on="contigId", right_on="contigId", how="left")
            df_join = df_join.drop("contigId")
        tsv_dir = self.output_dir.joinpath("tsv/")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--taxon', help='''tsv file containing bins taxonomic classification''')
    ...
    if args.annotation:
        exp.cluster_annotation(args.annotation, args.taxon)
