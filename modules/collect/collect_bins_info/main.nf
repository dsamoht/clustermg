process COLLECT {
    ...
    cat ${gtdbtkTsv} | sed '/^user_genome/d' | awk -F '\t' '{print \$2}' | awk -F ';' '{print \$1","\$2","\$3","\$4","\$5","\$6","\$7}' > taxa.tmp
    cat ${gtdbtkTsv} | sed '/^user_genome/d' | awk -F '\t' '{print \$8","\$11}' > info1.tmp
    cat ${gtdbtkTsv} | sed '/^user_genome/d' | awk -F '\t' '{print \$NF}' | sed 's/ /;/g' | sed 's/,/;/g' > info2.tmp
